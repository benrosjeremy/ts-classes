...
async function searchUsersByKeyword(keyword) {
    try {
        ...
        const matched = users.filter((user) => JSON.stringify(user).toLowerCase().includes(keyword.toLowerCase()));
        if (matched.length === 0) {
            showMessage("No users found with that keyword.");
        }
        else {
            resultsDiv.innerHTML = matched.map((user) => `<div><strong>${user.first_name} ${user.last_name}</strong><br/>${user.email}</div>`).join("<hr>");
        }
    }
    catch (error) {
